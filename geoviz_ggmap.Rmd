...
I perform a little tidying of the data, converting the `Date` column to a properly formatted date-time column using [`lubridate`](https://lubridate.tidyverse.org/) and extracting the weekday for future analysis.
...
```{r num-crimes, dependson = "tidy-crimes"}
nrow(crimes)
```
...
```{r plot-crime-point-alpha, dependson = "tidy-crimes"}
ggmap(chicago) +
  geom_point(data = crimes,
             aes(x = Longitude,
                 y = Latitude),
...
```{r kde-contour, dependson = "tidy-crimes"}
ggmap(chicago) +
...
```{r kde-fill, dependson = "tidy-crimes"}
ggmap(chicago) +
...
* `fill = ..level..` - the value for the `fill` aesthetic is the `level` calculated within `stat_density_2d()`, which we access using the `..` notation.
...
```{r plot-crime-wday, dependson = "tidy-crimes"}
ggmap(chicago) +
  stat_density_2d(data = crimes %>%
...
```{r homicide, dependson = "tidy-crime"}
(homicides <- crimes %>%
...
```{r violent, dependson = "tidy-crime"}
(violent <- crimes %>%
  filter(`Primary Type` %in% c("HOMICIDE", "CRIM SEXUAL ASSAULT", "ROBBERY")))
```
