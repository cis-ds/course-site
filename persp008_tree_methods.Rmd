...
titanic_tree_bag <- function(){
  ...
  err.rate.tree(titanic_tree, as_tibble(titanic_split$test)) - err.rate.rf(titanic_bag, as_tibble(titanic_split$test))
}
...
ggplot(tree_bag_comp, aes(err_diff)) +
  ...
  labs(x = "Tree error rate - bagged error rate")
```

On average, the decision tree test error rate is `r mean(tree_bag_comp$err_diff)` higher than the comparable bagged test error rate.
...
For our Titanic bagged model with all available predictors, we estimate an OOB error rate of `r tail(titanic_bag$err.rate[,1], n = 1)`.
