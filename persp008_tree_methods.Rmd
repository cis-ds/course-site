...
titanic_tree_bag <- function(){
  # split into training and test set
  titanic_split <- titanic %>%
    select(-Name, -Ticket, -Cabin, -Sex, -PassengerId) %>%
    mutate_each(funs(as.factor(.)), Pclass, Embarked) %>%
    na.omit %>%
    resample_partition(p = c("test" = .5, "train" = .5))
  
  # estimate model
  titanic_tree <- tree(Survived ~ ., data = titanic_split$train)
  titanic_bag <- randomForest(Survived ~ ., data = titanic_split$train,
                              mtry = 7, ntree = 500)
  
  # calculate difference in test MSE
  data_frame(err_rf = err.rate.rf(titanic_bag, as_tibble(titanic_split$test)),
             err_tree = err.rate.tree(titanic_tree, as_tibble(titanic_split$test)),
             auc_rf = auc(as_tibble(titanic_split$test)$Survived, predict(titanic_bag, newdata = as_tibble(titanic_split$test), type = "prob")[,2]),
             auc_tree = auc(as_tibble(titanic_split$test)$Survived, predict(titanic_tree, newdata = as_tibble(titanic_split$test), type = "vector")[,2]))
}

tree_bag_comp <- rerun(100, titanic_tree_bag()) %>%
  bind_rows() %>%
  mutate(err_diff = err_rf - err_tree,
             auc_diff = auc_rf - auc_tree)

ggplot(tree_bag_comp, aes(err_diff)) +
  geom_histogram() +
  geom_vline(xintercept = mean(tree_bag_comp$err_diff)) +
  labs(x = "Bagged error rate - Tree error rate",
       caption = "Lower values are better for bagging")

ggplot(tree_bag_comp, aes(auc_diff)) +
  geom_histogram() +
  geom_vline(xintercept = mean(tree_bag_comp$auc_diff)) +
  labs(x = "Bagged AUC - Tree AUC",
       caption = "Lower values are better for bagging")
```

On average, the decision tree test error rate is `r mean(tree_bag_comp$err_diff)` higher than the comparable bagged test error rate and the AUC difference is `r mean(tree_bag_comp$auc_diff)`.
...
For classification trees, larger values are better. So for the Titanic bagged model, gender, age, and fare are the most important predictors, wherease number of siblings/parents aboard and the port of departure are relatively unimportant.
...
The OOB error rate is a couple points smaller on the random forest model. Also we can observe that the average decrease in the Gini index associated with each variable is generally smaller using the random forest method compared to bagging - this is because of the variable restriction imposed when considering splits.
...
Rather than learning hard and fast like in bagging and random forests, boosting **learns slowly** over time as new trees are added. Boosting is a general process that can be used for other statistical learning methods. The three main tuning parameters when boosting are:
