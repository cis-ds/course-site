...
To estimate the $RMSE$, we can use `modelr::rmse()`
...
loocv_rmse <- map2_dbl(loocv_models, loocv_data$test, rmse)
mean(loocv_rmse)
```
...
titanic_models <- map(titanic_loocv$train, ~ glm(Survived ~ Age * Sex, data = .,
                                               family = binomial))
titanic_rmse <- map2_dbl(titanic_models, titanic_loocv$test, rmse.glm)
