...
Boo math! Actually this is pretty intuitive. All we're doing is for each observation, calculating the difference between the actual and predicted values for $y$, squaring that difference, calculating the average across all observations (this is the **Root Mean Squared Error** [MSE]), then taking the square root of that value. An $RMSE$ of 0 indicates the model perfectly predicted each observation. The larger the $RMSE$, the more error in the model. By taking the RMSE instead of just the MSE, the resulting value is in the original units of the response variable.
...
* I also use the `glm()` function rather than `lm()` - if you don't change the `family` parameter, the results of `lm()` and `glm()` are exactly the same.^[The default `family` for `glm()` is `gaussian()`, or the **Gaussian** distribution. You probably know it by its other name, the [**Normal** distribution](https://en.wikipedia.org/wiki/Normal_distribution).] I do this because I want to use a cross-validation function later that only works with results from a `glm()` function.
...
To estimate the $RMSE$ for a single partition (i.e. for a training or test set), we can use `modelr::rmse()`

```{r rmse, dependson="auto_split"}
rmse(auto_lm, auto_test)
```

For a strictly linear model, the $RMSE$ for the test set is `r formatC(rmse(auto_lm, auto_test), digits = 4)`. How does this compare to a quadratic model? We can use the `poly()` function in conjunction with a `map()` iteration to estimate the $RMSE$ for a series of models with higher-order polynomial terms:

```{r rmse_poly, dependson="auto_split"}
auto_poly_results <- data_frame(terms = 1:5,
           model = map(terms, ~ glm(mpg ~ poly(horsepower, .), data = auto_train)),
           RMSE = map_dbl(model, rmse, data = auto_test))

ggplot(auto_poly_results, aes(terms, RMSE)) +
  geom_line() +
...
Based on the $RMSE$ for the validation (test) set, a polynomial model with a quadratic term ($\text{horsepower}^2$) produces the lowest average error. Adding cubic or higher-order terms is just not necessary.
...
rerun(10, mse_variable(Auto)) %>%
  bind_rows(.id = "id") %>%
  ggplot(aes(terms, RMSE, color = id)) +
  geom_line() +
  labs(title = "Variability of RMSE estimates",
...
An alternative method is **leave-one-out cross validation** (LOOCV). Like with the validation set approach, you split the data into two parts. However the difference is that you only remove one observation for the test set, and keep all remaining observations in the training set. The statistical learning method is fit on the $n-1$ training set. You then use the held-out observation to calculate the $RMSE = \sqrt{(y_1 - \hat{y}_1)^2}$ which should be an unbiased estimator of the test error. Because this $RMSE$ is highly dependent on which observation is held out, **we repeat this process for every single observation in the data set**. Mathematically, this looks like:
...
Now we estimate the linear model $k$ times, excluding the holdout test observation, then calculate the **mean squared error**, not the **root** mean squared error. Why do it this way? Remember the formula for 
...
The results of the mapped `rmse()` function is the $RMSE$ for each iteration through the data, so there is one $RMSE$ for each observation. Calculating the `mean()` of that vector gives us the LOOCV RMSE.
...
for(i in terms){
  ...
  loocv_mse <- map2_dbl(loocv_models, loocv_data$test, mse)
  cv_error[[i]] <- sqrt(mean(loocv_mse))
}

cv_rmse <- data_frame(terms = terms,
           cv_RMSE = cv_error)
cv_rmse

ggplot(cv_rmse, aes(terms, cv_RMSE)) +
  geom_line() +
  labs(title = "Comparing quadratic linear models",
...
Let's use classification to validate the interactive terms model from before. For technical reasons, we need to use a custom `rmse.glm()` function to properly calculate the RMSE for binary response variables:
...
sqrt(mean(titanic_mse, na.rm = TRUE))
```
...
for(i in terms){
  ...
  cv_error_fold10[[i]] <- sqrt(mean(cv10_mse))
}
...
data_frame(terms = terms,
  ...
  gather(method, RMSE, loocv:fold10) %>%
  ggplot(aes(terms, RMSE, color = method)) +
  geom_line() +
...
profvis({
  ...
  for(i in terms){
    loocv_models <- map(loocv_data$train, ~ lm(mpg ~ poly(horsepower, i), data = .))
    loocv_rmse <- map2_dbl(loocv_models, loocv_data$test, mse)
    cv_error[[i]] <- sqrt(mean(loocv_mse))
  }
})
```
...
profvis({
  ...
  for(i in terms){
    cv10_models <- map(cv10_data$train, ~ lm(mpg ~ poly(horsepower, i), data = .))
    cv10_rmse <- map2_dbl(cv10_models, cv10_data$test, rmse)
    cv_error_fold10[[i]] <- mean(cv10_rmse)
  }
})
```
...
titanic_models <- map(titanic_kfold$train, ~ glm(Survived ~ Age * Sex, data = .,
                                               family = binomial))
titanic_rmse <- map2_dbl(titanic_models, titanic_kfold$test, rmse.glm)
mean(titanic_rmse, na.rm = TRUE)
```
