...
Suppose you want to know the American population's attitudes [rocky road ice cream](https://en.wikipedia.org/wiki/Rocky_road_(ice_cream)).

![](https://upload.wikimedia.org/wikipedia/commons/thumb/1/1c/Rockyroadicecream.jpg/800px-Rockyroadicecream.jpg)

Americans rate their favorability towards rocky road ice cream using a [feeling thermometer](http://link.springer.com/referenceworkentry/10.1007%2F978-94-007-0753-5_1028) with a range of 0-100. Favorability rating is defined as $X$. Furthermore suppose the population mean $\mu$ is in truth 45 and the standard deviation $s.d.(X)$ is 4. But in the real world we do not know the population mean and standard deviation. We instead generate estimates for the mean $\hat{\mu}$ and standard deviation $\widehat{s.d.(X)}$ by surveying a sample of the population. Let's simulate a survey of 1000 individuals using the `rnorm()` function:

```{r ice-sim}
(rocky_road <- round(rnorm(1000, mean = 45, sd = 15)))
```
