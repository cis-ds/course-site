...
* Estimate an OLS model and obtain the residuals
* Divide each of the squared residuals by $\frac{RSS}{N}$, aka the empirical estimate of $\hat{\sigma}^2$. Call the resulting things $p_i$
* Regress $p_i$ against either:
    * The predicted values $\hat{y}_i$, or
    * All the $k$ variables you think might be causing the heteroscedasticity
* Take the MSS of the resulting regression and divide it by 2
    * The resulting statistic follows a $\chi^2$ distribution with either 1 (for the fitted-values test) or $(k-1)$ (for the all-variables version) degrees of freedom
