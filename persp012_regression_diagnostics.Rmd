...
We see some mild changes in the estimated parameters, but drastic reductions in the standard errors. The problem is that this reduction is potentially biased through the estimated covariance matrix because the sampling error in the estimates should reflect hte additional source of uncertainty, which is not explicitly accounted for just basing it on the original residuals. Instead, it would be better to model the weights as a function of relevant explanatory variables.^[Omitted here for time purposes. You can find details on this estimation procedure on the internet.]
...
Now let's say we want to recode `displ` so it is centered around it's mean and reestimate the model:
...
Ooops. What's the problem? `disp` and `disp_mean` are perfectly correlated with each other:
...
**Shrinkage methods** involve fitting a model involving all $p$ predictors and shrinking the estimated coefficients towards zero. This shrinkage reduces variance in the model. Whem multicollinearity is high, the variance of the estimator $\hat{\beta}$ is also high. By shrinking the estimated coefficient towards zero, we may increase **bias** in exchange for smaller variance in our estimates.
