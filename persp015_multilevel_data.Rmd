...
## Multilevel Data Structures & Problems With Classical Regression
...
Instead, multilevel modeling uses a compromise between these two approaches. A **soft constraint** is applied to the coefficients: they are assigned a probability distribution. A varying intercept model could take the following form:
...
with variation in the $\alpha_{j}$'s and the $\beta_{j}$'s and also a between-group correlation parameter. We can also include group-level predictors:
...
ols <- lm(y ~ x, data = sim_data_mlm)
mlm <- lmer(y ~ x + (1|groups), data = sim_data_mlm)

# plot data
p <- ggplot(sim_data_mlm, aes(x, y)) +
...
p +
  ...
  geom_line(data = add_predictions(sim_data_mlm, mlm), aes(x, pred, color = factor(groups)))
```
...
```{r final}
final <- glmer(defect ~ pid_abs + feel_def + defect_cor + inc + (1 + feel_def|year),
               data = anes, family = binomial(link = "logit"))
tidy(final)
...
for(i in 1:ncol(ranef(final)$year)){
  se.ran[,i] <- apply(sim.final@ranef$year[,,i], 2, function(x) sd(x, na.rm = TRUE))
}

se.ran <- cbind(se.ran[,1],0,se.ran[,2],0,0)
...
## Limitations of MLM for this application
