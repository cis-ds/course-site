...
We can [decompose the problem](block003_transform-data.html#computer_programming_as_a_form_of_problem_solving) into a series of discrete steps:
...
* Intermediate steps
* Overwrite the original (make sure to save a copy of `gun_deaths` as `gun_deaths2` for this code chunk)
...
```{r intermediate_gd}
gun_deaths1 <- filter(gun_deaths, sex == "M", race == "White")
...
gun_deaths2 <- filter(gun_deaths2, sex == "M", race == "White")
...
summarize(
  group_by(
    filter(gun_deaths, sex == "M", race == "White"),
    education),
  n = n()
)
```
