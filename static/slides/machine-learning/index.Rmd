...
ggplot(data = bechdel, aes(
  ...
  labs(
    x = "Metacritic score",
    y = "Domestic gross (in millions of 2013 dollars)",
    color = "Bechdel test"
  ) +
...
```{r}
lr_mod %>% 
  fit(bechdel ~ metascore + domgross_2013, 
      data = bechdel) %>% 
  broom::tidy()
```
]
...
lr_preds <- lr_mod %>% 
  ...
  bind_cols(bechdel) %>% 
  mutate(.pred_match = if_else(bechdel == .pred_class, 1, 0))
```
...
```{r results='hide'}
fit(tree_mod, bechdel ~ metascore + domgross_2013, data = bechdel)
```
...
tree_mod %>%                               # parsnip model
  fit(bechdel ~ metascore + domgross_2013,   # a formula
...
```{r}
bechdel_split <- initial_split(data = bechdel, strata = bechdel, prop = .9)
bechdel_split
```
...
for (i in 1:10) {
  ...
  acc[i] <-
    lr_mod %>% 
      fit(bechdel ~ metascore + domgross_2013, data = new_train) %>% 
      predict(new_test) %>% 
...
tree_mod %>% 
  fit_resamples(
    bechdel ~ metascore + domgross_2013, 
...
set.seed(100)
lr_mod %>% 
  fit_resamples(bechdel ~ metascore + domgross_2013, 
                resamples = bechdel_folds) %>% 
  collect_metrics()
```
...
```{r}
tree_mod %>% 
  fit(bechdel ~ metascore + domgross_2013, data = bechdel_train) %>% 
