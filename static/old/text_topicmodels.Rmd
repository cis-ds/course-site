...
```{r assignments3, dependson = "assignments2"}
assignments %>%
...
```{r flopson, dependson = "word_counts"}
word_counts %>%
  filter(word == "flopson")
```
...
```{r ap_lda_compare, dependson="associated_press", eval = FALSE}
n_topics <- c(2, 4, 10, 20, 50, 100)
ap_lda_compare <- n_topics %>%
  map(LDA, x = ap_dtm, control = list(seed = 1234))
```

```{r ap_lda_compare_real, dependson="associated_press"}
n_topics <- c(2, 4, 10, 20, 50, 100)
...
```{r ap-10-ldavis, dependson = "ap_lda_compare"}
ap_10_json <- topicmodels_json_ldavis(fitted = ap_lda_compare[[3]],
                                       doc_term = ap_dtm)
```

```{r ap-10-ldavis-serve, dependson = "ap-100-ldavis", eval = FALSE}
serVis(ap_10_json)
```
