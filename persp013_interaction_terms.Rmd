...
nes %>%
  ...
  geom_point() +
  labs(title = "Rxpected Obama thermometer score",
...
beta.hat <- coef(obama_ideo_gop)
cov <- vcov(obama_ideo_gop)

# possible set of values for z
z <- seq(min(nes$RConserv), max(nes$RConserv))

# calculate instantaneous effect
dy.dx <- beta.hat[["GOP"]] + beta.hat[["RConserv:GOP"]] * z

# calculate standard errors for instantaeous effect
se.dy.dx <- sqrt(cov["GOP", "GOP"] + z^2 * cov["RConserv:GOP", "RConserv:GOP"] + 2 * z * cov["GOP", "RConserv:GOP"])

# combine into data frame
data_frame(dy.dx = dy.dx,
           se = se.dy.dx)
