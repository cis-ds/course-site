...
## Most frequent words, by book {.scrollable}

```{r word-freq, echo = FALSE, fig.asp = 1.5}
hp_words %>%
  # delete stopwords
  ...
  filter(top <= 15) %>%
  # create barplot
  ggplot(aes(x = -top, y = n / 1000, fill = book)) + 
  geom_col(color = "black") +
  # print words in plot instead of as axis labels
  geom_text(aes(label = word), hjust = "left", nudge_y = .1) +
  labs(title = "Most frequent words in Harry Potter",
       x = NULL,
       y = "Word count (in thousands)") +
