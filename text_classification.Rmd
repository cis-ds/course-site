---
title: "Text analysis: supervised classification"
output:
  html_document:
    toc: true
    toc_float: true
...
1. Once you have trained a model with sufficient predictive accuracy, apply the model the remaining set of documents that have never been hand-coded (say $1000000$)
...
Otherwise everything else is the same as before. Notice how long it takes to build a random forest model with 10 trees, compared to a more typical random forest model with 500 trees
...
```{r rf-train-500}
system.time({
  congress_rf_500 <- train(x = as.matrix(congress_dtm),
                           y = factor(congress$major),
                           method = "rf",
                           ntree = 500,
                           trControl = trainControl(method = "oob"))
...
```{r rf-diag}
congress_rf_500$finalModel
varImpPlot(congress_rf_500$finalModel)
```
