...
> You will need to install the development version of `ggplot2` in order to implement the approach described below. You can do this by running `devtools::install_github("tidyverse/ggplot2")` from the console.
...
That is, each row defines a boundary point. The boundary for a state is created by drawing a line connecting each point. We need to convert this points-based data frame to a simple features data frame where each feature is a state.^[Modified from [Convert a fortified data.frame back to sf object](https://stackoverflow.com/questions/49526198/convert-a-fortified-data-frame-back-to-sf-object) on StackOverflow.] First we convert the data frame to a simple features data frame using `st_as_sf()`. This creates a simple features data frame where each feature is a boundary point, like before.

```{r fiftystater-sf-point}
...
Finally, we combine together all the pieces of land comprising each state. For example, Alabama is a single contiguous landmass, whereas Alaska is `r st_as_sf(fifty_states, coords = c("long", "lat")) %>% group_by(id, piece) %>% summarize(do_union = FALSE) %>% st_cast("POLYGON") %>% filter(id == "alaska") %>% nrow()` distinct pieces of land. This is another `group_by() %>% summarize()` operation, but only aggregating on `id`.

```{r fiftystater-sf-multipolygon}
...
Finally we set the projection method^[Run `?st_crs` from the console to read the documentation for this function and understand how this number is determined.] and draw the map:

```{r fiftystater-proj}
st_crs(sf_fifty) <- 4326
```

```{r fiftystater-sf-plot}
