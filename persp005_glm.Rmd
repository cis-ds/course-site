...
$$\Pr(Y_i \leq j) = \Pr(\zeta_i \leq \alpha_j)$$

$$\Pr(Y_i \leq j) = \Pr(\alpha + \beta_{1}X_{i1} + \dots + \beta_{k}X_{ik} \leq \alpha_j)$$

$$\Pr(Y_i \leq j) = \Pr(\epsilon_i \lt \alpha + \beta_{1}X_{i1} + \dots + \beta_{k}X_{ik})$$

If the errors follow a logistic distribution, then we have an ordered logistic regression model:
