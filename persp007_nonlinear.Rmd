...
data_frame(terms = c(1, 5, 10),
           models = map(terms, ~ glm(y ~ bs(x, df = .), data = sim_piece)),
...
We can also see how the spline imposes a local structure by comparing it to the related polynomial regression model:

```{r sim-spline-compare}
# models
sim_piece_global <- glm(y ~ poly(x, 5, raw = TRUE), data = sim_piece)
sim_piece_local <- glm(y ~ bs(x, df = 5), data = sim_piece)

# graph
sim_piece_cont %>%
  data_grid(x = seq(0, 10, by = .001)) %>%
  gather_predictions(sim_piece_global, sim_piece_local) %>%
  ggplot(aes(x, pred)) +
  geom_point(data = sim_piece, aes(y = y)) +
  geom_line(aes(y = pred, color = model), size = 1) +
  geom_vline(xintercept = 5, linetype = 2, color = "grey") +
  labs(title = "Piecewise cubic regression",
       x = "X",
