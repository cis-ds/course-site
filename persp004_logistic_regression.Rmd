...
The result is two lines that are parallel to each other. The addition of the gender parameter acts like an second intercept when we compare age to probability of survival. All that happens is the line shifts up or down based on the estimated gender parameter.
...
$$p(\text{Survival}_{female}) = \frac{e^{\beta_0 + \beta_{1}\text{Age} + \beta_{3} \times \text{Age} \times 1}}{1 + e^{\beta_0 + \beta_{1}\text{Age} + \beta_{3} \times \text{Age} \times 1}}$$

which reduces to

$$p(\text{Survival}_{female}) = \frac{e^{\beta_0 + \beta_{1}\text{Age} + \beta_{3} \times \text{Age}}}{1 + e^{\beta_0 + \beta_{1}\text{Age} + \beta_{3} \times \text{Age}}}$$

$$p(\text{Survival}_{female}) = \frac{e^{\beta_0 + (\beta_{1} + \beta_{3})\text{Age}}}{1 + e^{\beta_0 + (\beta_{1} + \beta_{3})\text{Age}}}$$
...
$$PRE_{\text{Age}} = \frac{`r sum(survive_age_woman_x$y != median(survive_age_woman_x$y))` - `r sum(survive_age_woman_x$y != round(survive_age_woman_x$fitted.values))`}{`r sum(survive_age_woman_x$y != median(survive_age_woman_x$y))`}$$

$$PRE_{\text{Age}} = \frac{`r sum(survive_age_woman_x$y != median(survive_age_woman_x$y)) - sum(survive_age_woman_x$y != round(survive_age_woman_x$fitted.values))`}{`r sum(survive_age_woman_x$y != median(survive_age_woman_x$y))`}$$

$$PRE_{\text{Age}} = `r formatC((sum(survive_age_woman_x$y != median(survive_age_woman_x$y)) - sum(survive_age_woman_x$y != round(survive_age_woman_x$fitted.values))) / sum(survive_age_woman_x$y != median(survive_age_woman_x$y)) * 100, digits = 3)`\%$$
...
A receiver operating characteristic (ROC) curve is an alternative graphical method for comparing the types of errors for differing thresholds. In this graph, you plot the false positive rate vs. the true positive rate (i.e. 1 - sensitivity vs. specificity).
...
library(pROC)
auc(x_accuracy$pred, x_accuracy$Survived)
```

With an AUC of `r formatC(auc(x_accuracy$pred, x_accuracy$Survived), digits = 3)`, this model performs decently.
