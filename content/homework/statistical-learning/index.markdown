...
1. Estimate a basic (single variable) linear regression model of the relationship between the importance of the video and feelings towards Donald Trump. That is, predict an individual's attitude towards Donald Trump based on their response to the question about the Access Hollywood video. Visualize the model using `ggplot()` and determine whether there appears to be a significant relationship.
