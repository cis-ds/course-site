...
```{r appa-4-topn, dependson = "appa-topic-4"}
...
```{r appa-topic-12, dependson = "appa-dtm"}
appa_lda12 <- LDA(appa_dtm, k = 12, control = list(seed = 123))
appa_lda12
```

```{r appa-12-topn, dependson = "appa-topic-12"}
...
```{r appa-topic-12-perplex, dependson="appa-topic-12"}
...
```{r appa-lda-compare, dependson = "appa-dtm"}
...
```{r appa_lda_compare_viz, dependson="appa_lda_compare"} 
tibble(
  k = n_topics,
...
```{r appa-100-topn, dependson = "appa-lda-compare"}
...
```{r topicmodels-lda-vis, message = FALSE}
