...
The short report printed gives the file name, mentions that there are `r nrow(nyc_shape)` features (records, represented as rows) and `r ncol(nyc_shape)` fields (attributes, represented as columns), states that the spatial data file is a `MULTIPOLYGON`, provides the bounding box coordinates, and identifies the projection method (which we will discuss later). If we print the first rows of `nyc_shape`:
