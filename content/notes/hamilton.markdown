...
```r
hamilton_cor <- hamilton_tidy %>%
  group_by(word) %>%
  filter(n() > 20) %>%
  pairwise_cor(item = word, feature = song_number)
arrange(hamilton_cor, -correlation)
```
...
##  1 dat       da              1    
##  2 da        dat             1    
##  3 jefferson thomas          0.733
##  4 thomas    jefferson       0.733
##  5 helpless  angelica        0.587
##  6 angelica  helpless        0.587
##  7 throwing  shot            0.577
##  8 shot      throwing        0.577
##  9 stand     gonna           0.575
## 10 gonna     stand           0.575
