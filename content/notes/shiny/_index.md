...
The raw dataset contains information about all employees of the city of Chicago (`employees-all.csv`). The processed dataset we'll be using in this app is the subset of employees who are **wage** employees (paid hourly), as opposed to **salaried** employees. This subset is in the `employees-wage.csv` file.[^salary]
...
Fixing this is (relatively) simple. Inside the `employ_filter` reactive function, we should check if the department input exists, and if not then just not filter for that column. It's easier to do this if we do not write `employ_filter` as a single piped operation:[^possible]
...
[^salary]: Feel free to explore the full employee dataset for [homework 11](/homework/shiny/), but we will only focus on the wage employees today because there are different variables relevant to wage vs. salaried employees. We don't want to get into that confusion today.
