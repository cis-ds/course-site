...
employ <- read_csv("https://raw.githubusercontent.com/cis-ds/shiny-demo/master/employees-wage.csv")
```
...
Different departments will offer different wage structures depending on the value of the skills in demand. The city classifies wage employees into `r n_distinct(employ$department)` distinct departments:

```{r departments, echo = FALSE}
count(employ, department) %>%
  knitr::kable(col.names = c("Department", "Number of Employees"))
```
...
We can use this concept in our app to populate the choices for the job title selector. `employ$job_title` contains `r n_distinct(employ$job_title)` distinct job titles, not all of which are applicable to every department. It would be helpful if we allow app users to filter the dataset by job title, we only allow as an input job titles which fall within the specified department(s).
