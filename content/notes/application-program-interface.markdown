...
`mp_corpus()` can be used to download the original manifestos as full text documents stored as a [**corpus**](text001_workflow.html#extract_documents_and_move_into_a_corpus). Once you obtain the corpus, you can perform [text](cm017.html) [analysis](cm018.html). As an example, let's compare the most common words in the Democratic and Republican Party manifestos from the 2012 U.S. presidential election:
...
`tidycensus` also can return [simple feature geometry](geoviz_import_data.html) for geographic units along with variables from the decennial Census or ACS, which can then be [visualized using `geom_sf()`.](geoviz_plot.html) Let's look at median household incomeby Census tracts from the 2012-2016 ACS in Loudoun County, Virginia:
