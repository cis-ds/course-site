...
![[tags](https://xkcd.com/1144/)](https://medias.xkcd.com/comics/tags.png)
...
![IMDB page for *Frozen*](/media/frozen_bell.png)

{{< spoiler text="Click for the solution" >}}

![HTML tag for "Kristen Bell"](/media/frozen_span.png)
...
![HTML only](/media/shiny-css-none.png)

##### HTML + CSS

![HTML + CSS](/media/shiny-css.png)
...
1. Extract specific elements with `html_elements()`
1. Extract content from elements with various functions
...
* The first argument to `html_elements()` is the HTML document or a set of elements previously extracted from the document
* The second argument is a CSS selector to identify which elements to select
...
```
## [1] "Written by DeAlan Wilson for ComedyE.com"
```
...
1. Select the elements that are both `id = "titleCast"` and `a`s
1. Extract the text from the elements
...
library(rvest)
frozen <- read_html("http://www.imdb.com/title/tt2294629/")
cast <- html_elements(frozen, "#titleCast a")
html_text2(cast)
```
...
## [47] "Young Elsa"                                 
## [48] "See full cast"                              
## [49] "View production, box office, & company info"
```
...
```r
cast2 <- html_elements(frozen, "#titleCast td:nth-child(2) a")
html_text2(cast2)
```
