...
```{r pol-house}
# get data
house <- read_csv("data/dw-nominate-house.csv") %>%
  filter(party == 100 | party == 200) %>%
  mutate(year = 1789 + cong * 2 - 2,
         party = factor(party, levels = c(100, 200),
                        labels = c("Democrat", "Republican")))

# get median first-dimension score by party for each session
house %>%
  filter(cong > 20) %>%
  group_by(year, party) %>%
  summarize(dwnom1 = mean(dwnom1, na.omit = TRUE)) %>%
  ggplot(aes(year, dwnom1, color = party)) +
  geom_line() +
  scale_color_manual(values = c("blue", "red")) +
  labs(title = "House 1829-2013",
       subtitle = "Party means on Liberal-Conservative Dimension",
       x = "Year",
       y = "Liberal - Conservative",
       color = NULL) +
  theme(legend.position = "bottom")

# compare first vs. second dimension for two years
house %>%
  filter(year == 1875) %>%
  ggplot(aes(dwnom1, dwnom2, color = party)) +
  geom_point() +
  scale_color_manual(values = c("blue", "red")) +
  labs(title = "House of Representatives (1875-76)",
       subtitle = "Legislators' ideal points",
...
house %>%
  ...
  geom_point() +
  scale_color_manual(values = c("blue", "red")) +
  labs(title = "House of Representatives (2013-14)",
       subtitle = "Legislators' ideal points",
       x = "Liberal - Conservative",
       y = "Issue of the day",
       color = NULL) +
...
```{r pol-senate}
# get data
senate <- read_csv("data/dw-nominate-senate.csv") %>%
  filter(party == 100 | party == 200) %>%
  mutate(year = 1789 + cong * 2 - 2,
         party = factor(party, levels = c(100, 200),
                        labels = c("Democrat", "Republican")))
